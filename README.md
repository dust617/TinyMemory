...
| 文件 | 作用 |
|---|---|
| `index.ts` | pi-coding-agent 扩展：自动注入有界 Brief、`memory-recall/save/review/status` 等工具路由、观察事件（tool_failure / config_change / distilled）入 INBOX |
...
| `scripts/check-memory.mjs` | 项目记忆全量校验器（`npm run memory:check`）：字节/行上限、TTL、Fact ID/Replaces、遗留 KEYSTORE、秘密模式、归档总量 |
...
| `.pi/memory/INBOX.jsonl` | 观察候选（需人工 review 后入 FACTS） | audit 保留 40 条 / distilled 保留 60 条 |
